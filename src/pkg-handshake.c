...
int pc__handshake_req(pc_client_t *client) {
  ...
  if(body == NULL) {
    LOGD( "Fail to create json_t for handshake request.\n");
    return -1;
  }
  ...
  if(data == NULL) {
    LOGD( "Fail to compose Pomelo handshake package.\n");
    goto error;
  }
  ...
  if(pc__binary_write(client, buf.base, buf.len, pc__handshake_req_cb)) {
    LOGD( "Fail to send handshake request.\n");
    goto error;
  }
...
static int pc__handshake_ack(pc_client_t *client) {
  ...
  if(buf.len == -1) {
    LOGD( "Fail to encode handshake ack package.\n");
    goto error;
  }

  if(pc__binary_write(client, buf.base, buf.len, pc__handshake_ack_cb)) {
    LOGD( "Fail to send handshake ack.\n");
    goto error;
  }
...
static void pc__handshake_req_cb(uv_write_t* req, int status) {
  ...
  if(PC_TP_ST_WORKING != transport->state) {
    LOGD( "Invalid transport state for handshake req cb: %d.\n",
            transport->state);
    return;
  }

  if(status == -1) {
    LOGD( "Fail to write handshake request async, %s.\n",
...
static void pc__handshake_ack_cb(uv_write_t* req, int status) {
  ...
  if(PC_TP_ST_WORKING != transport->state) {
    LOGD( "Invalid transport state for handshake ack cb: %d.\n",
            transport->state);
    return;
  }

  if(status == -1) {
    LOGD( "Fail to write handshake ack async, %s.\n",
            uv_err_name(uv_last_error(client->uv_loop)));
    pc_client_stop(client);
  } else {
